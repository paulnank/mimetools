...
void convertAsciiToBase64(size_t wrapLength, bool padFlag, bool byLineFlag)
{
    ...
    size_t bufferLength = (selectedLength + 2) / 3 * 4 + 1; // assume normal ratio to encode 3 characters as 4 bytes
    if (byLineFlag)
    {
        if (padFlag)
        {
            bufferLength = (selectedLength + 1) / 2 * 5 + 1; // extreme case 2 bytes per line becomes 5 characters with padding
        }
        else
        {
            bufferLength = (selectedLength + 1) / 2 * 3 + 1; // without padding 2 bytes per line may become 3 characters
        }
    }
    if (wrapLength > 0)
    {
...
void convertBase64ToAscii(bool strictFlag, bool noWhiteSpace)
{
    HWND hCurrScintilla = getCurrentScintillaHandle();
    size_t selectedLength = ::SendMessage(hCurrScintilla, SCI_GETSELTEXT, 0, 0);
    if (selectedLength == 0) return;
    ...
    int len = base64Decode(decodedText, selectedText, selectedLength - 1, strictFlag, noWhiteSpace);
